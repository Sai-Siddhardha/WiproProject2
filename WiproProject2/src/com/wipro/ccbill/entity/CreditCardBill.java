	...
	public String validateData() throws InputValidationException
	{
		try
		{
			if(creditCardNo.equals("null"))
				throw new InputValidationException("CreditCard Number is null");
			if(creditCardNo.length()<16||creditCardNo.length()>16)
				throw new InputValidationException("CreditCard Number is not exactly 16");
			if(customerId.equals("null"))
				throw new InputValidationException("Customer Id is null");
			if(customerId.length()<6)
				throw new InputValidationException("Customer Id should be greater than 6");
		}
		catch(InputValidationException ive)
		{
			return "not valid";
